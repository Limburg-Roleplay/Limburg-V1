...
AddEventHandler(
    ...
    function(loodsId)
        ...
        MySQL.Async.execute(
            "UPDATE loodsen SET owner = NULL, inhabitants = NULL, opslag = 0, upgrades = 1, current_inhabitants = NULL, plants = NULL WHERE id = @id",
            {
                ["@id"] = loodsId
            },
            function(rowsChanged)
    ...
    MySQL.Async.execute('UPDATE loodsen SET opslag = opslag - @minSellAmount WHERE owner = @owner AND opslag >= @minSellAmount', {
        ...
        if rowsChanged > 0 then
            MySQL.Async.fetchScalar('SELECT opslag FROM loodsen WHERE owner = @owner', {
                ...
                local blackMoneyAmount = math.random(150000, 200000)
