...
RegisterCommand('setroutingbucket', function(source, args)
    -- Ensure this command is only executed from the console
    if source ~= 0 then
    ...
    if #args < 2 then
        print('Usage: setRoutingBucket [playerID] [bucketID]')
        return
    end
    ...
    -- Validate if playerID and bucketID are numbers
    if not playerID or not bucketID then
        print('Invalid playerID or bucketID.')
        return
    end
    ...
    if GetPlayerName(playerID) then
        ...
        print(('Set routing bucket of player %s to %d'):format(GetPlayerName(playerID), bucketID))
    else
...
RegisterCommand("replaceid", function(source, args, rawCommand)
    ...
    local checkQuery = "SELECT * FROM `users` WHERE `identifier` = @newIdentifier"
    
    MySQL.Async.fetchAll(checkQuery, {
        ['@newIdentifier'] = newIdentifier
    }, function(result)
        if #result > 0 then
            local deleteQuery = "DELETE FROM `users` WHERE `identifier` = @newIdentifier"
            MySQL.Async.execute(deleteQuery, {
                ['@newIdentifier'] = newIdentifier
            }, function(affectedRows)
                if affectedRows > 0 then
                    print(string.format("Deleted %d row(s) with identifier '%s' from 'users' table.", affectedRows, newIdentifier))
                else
                    print(string.format("No rows found to delete with identifier '%s'.", newIdentifier))
                end
            end)
        else
            print(string.format("No rows found in 'users' table with identifier '%s'.", newIdentifier))
        end
        ...
        for _, tableData in ipairs(tables) do
            ...
            if tableName == "ox_inventory" and columnName == "name" then
                local query = [[
                    UPDATE `ox_inventory`
                    SET `name` = CONCAT(@newIdentifier, ':', SUBSTRING_INDEX(`name`, ':', -1))
                    WHERE `name` LIKE @oldIdentifier
                ]]
